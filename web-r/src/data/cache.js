...
import { Cache, notifyMessage } from '../utils';
...
class AppData extends Cache {
  static Context = React.createContext(null);
  static DataContext = React.createContext(null);
  static MapsContext = React.createContext({});

  static propTypes = {
    beginMapLoad: PropTypes.func.isRequired,
    onMapProgress: PropTypes.func.isRequired,
    finishMapLoad: PropTypes.func.isRequired,
    failMapLoad: PropTypes.func.isRequired,
    ...
    readFile(file).then(map => {
      ...
      this.parser.parse(meta, map)
        .then(data => {
          ...
          let id = 1;
          while (Object.prototype.hasOwnProperty.call(this.maps, `map${id}`)) {
            id += 1;
          }
          id = `map${id}`;
