...
export default class Panel extends Component {
    ...
    return React.Children.map(children, (child, index) => {
      ...
      if (resizable || prevResizable) {
        if (firstResizable < index && lastResizable >= index) {
          result = [<Resizer key={`resizer-${index}`} row={rows} col={cols} onResize={delta => this.onResize(index, delta)}/>, result];
        }
      }
  ...
  render() {
    ...
    const panel = (
      <div ref={this.onRef} className={classNames('Panel', {rows, cols}, className)} {...props}>
