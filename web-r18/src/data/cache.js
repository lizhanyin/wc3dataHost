    ...
    readFile(file).then(map => {
      ...
      this.parser.parse(meta, map)
        .then(data => {
          ...
          let nid = 1;
          while (Object.prototype.hasOwnProperty.call(this.maps, `map${nid}`)) {
            nid += 1;
          }
          ...
          if (this.dataStore) {
            Promise.all([
              this.nameStore.set(sid, file.name),
              this.dataStore.set(sid, new Blob([data], {type: 'application/octet-stream'}))
            ]).catch(() => {
              this.nameStore.remove(sid);
              this.dataStore.remove(sid);
            });
          }

          this.maps = {...this.maps, [sid]: file.name};
          this.mapData_[sid] = loadArchive(data).then(arc => new MapData(this, arc, sid, file.name))
            .catch(err => Promise.reject(notifyMessage(err.toString(), 'danger')));

          this.root.finishMapLoad(sid);
        })
        .catch(err => {
          delete this.parser;
